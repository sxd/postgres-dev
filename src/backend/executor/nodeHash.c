...
ExecHashTableCreate(HashState *state)
{
	...
	if (state->want_bloom_filter)
	{
		...
		/*
		 * Note: bloomFilterChecked/Rejected are deliberately not reset here.
		 * The filter is rebuilt on each rescan, but EXPLAIN ANALYZE reports
		 * totals across the whole execution; zeroing per rebuild made the
		 * producer's numbers disagree with the recipient's cumulative ones.
		 * They start at zero from ExecInitHash's makeNode.
		 */
